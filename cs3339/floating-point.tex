...
\begin{enumerate}
  ...
  \item \colorbox{yellow!40}{Convert the \textbf{fractional part} to binary (repeated multiplication
        by 2).}
...
\begin{enumerate}
  ...
  \item \colorbox{yellow!40}{Fractional part $0.625$:}
        \colorbox{yellow!40}{$0.625\times2=1.25$ (1),}
        \colorbox{yellow!40}{$0.25\times2=0.5$ (0),}
        \colorbox{yellow!40}{$0.5\times2=1.0$ (1).}
        \colorbox{yellow!40}{So $0.625_{10} = 0.101_2$.}
...
\begin{center}
\begin{tikzpicture}[
  bit/.style={draw, minimum height=0.55cm, font=\ttfamily\scriptsize, anchor=west},
]
% +0
\node[font=\small\bfseries, anchor=east] at (-0.1, 0) {$+0.0$:};
\node[bit, minimum width=0.7cm, fill=red!15]   (s1) at (0, 0) {0};
\node[bit, minimum width=2.8cm, fill=blue!15]  (e1) at (s1.east) {0000\;0000};
\node[bit, minimum width=5.5cm, fill=green!15] (f1) at (e1.east) {0000\;0000\;0000\;0000\;0000\;000};

% -0
\node[font=\small\bfseries, anchor=east] at (-0.1, -0.8) {$-0.0$:};
\node[bit, minimum width=0.7cm, fill=red!30]   (s2) at (0, -0.8) {1};
\node[bit, minimum width=2.8cm, fill=blue!15]  (e2) at (s2.east) {0000\;0000};
\node[bit, minimum width=5.5cm, fill=green!15] (f2) at (e2.east) {0000\;0000\;0000\;0000\;0000\;000};
\end{tikzpicture}
\end{center}
\end{sub}
...
% +inf
\node[font=\small\bfseries, anchor=east] at (-0.1, 0) {$+\infty$:};
\node[bit, minimum width=0.7cm, fill=red!15]   (s1) at (0, 0) {0};
\node[bit, minimum width=2.8cm, fill=blue!30]  (e1) at (s1.east) {1111\;1111};
\node[bit, minimum width=5.5cm, fill=green!15] (f1) at (e1.east) {0000\;0000\;0000\;0000\;0000\;000};

% -inf
\node[font=\small\bfseries, anchor=east] at (-0.1, -0.8) {$-\infty$:};
\node[bit, minimum width=0.7cm, fill=red!30]   (s2) at (0, -0.8) {1};
\node[bit, minimum width=2.8cm, fill=blue!30]  (e2) at (s2.east) {1111\;1111};
\node[bit, minimum width=5.5cm, fill=green!15] (f2) at (e2.east) {0000\;0000\;0000\;0000\;0000\;000};
\end{tikzpicture}
\end{center}
\end{sub}
...
% qNaN
\node[font=\small\bfseries, anchor=east] at (-0.1, 0) {qNaN:};
\node[bit, minimum width=0.7cm, fill=red!15]   (s1) at (0, 0) {0};
\node[bit, minimum width=2.8cm, fill=blue!30]  (e1) at (s1.east) {1111\;1111};
\node[bit, minimum width=5.5cm, fill=green!30] (f1) at (e1.east) {\textbf{1}000\;0000\;0000\;0000\;0000\;000};

% sNaN
\node[font=\small\bfseries, anchor=east] at (-0.1, -0.8) {sNaN:};
\node[bit, minimum width=0.7cm, fill=red!15]   (s2) at (0, -0.8) {0};
\node[bit, minimum width=2.8cm, fill=blue!30]  (e2) at (s2.east) {1111\;1111};
\node[bit, minimum width=5.5cm, fill=green!30] (f2) at (e2.east) {\textbf{0}100\;0000\;0000\;0000\;0000\;000};
\end{tikzpicture}
\end{center}
Key difference: fraction bit [22] distinguishes qNaN (\textbf{1}) from sNaN (\textbf{0}).
\end{sub}
...
\node[font=\small\bfseries, anchor=east, align=right] at (-0.1, 0) {smallest\\denorm:};
\node[bit, minimum width=0.7cm, fill=red!15]   (s1) at (0, 0) {0};
\node[bit, minimum width=2.8cm, fill=blue!15]  (e1) at (s1.east) {0000\;0000};
\node[bit, minimum width=5.5cm, fill=green!30] (f1) at (e1.east) {0000\;0000\;0000\;0000\;0000\;00\textbf{1}};

% Largest denormalized
\node[font=\small\bfseries, anchor=east, align=right] at (-0.1, -0.8) {largest\\denorm:};
\node[bit, minimum width=0.7cm, fill=red!15]   (s2) at (0, -0.8) {0};
\node[bit, minimum width=2.8cm, fill=blue!15]  (e2) at (s2.east) {0000\;0000};
\node[bit, minimum width=5.5cm, fill=green!30] (f2) at (e2.east) {1111\;1111\;1111\;1111\;1111\;111};
\end{tikzpicture}
\end{center}
\end{sub}
...
\subsection{MIPS Data Path Overview}
\begin{sub}
An architectural map showing which hardware components exist, where they
live, and which instructions communicate between them.
...
% ============ Processor boundary ============
\draw[thick, rounded corners=8pt, fill=black!3]
  (-0.5, 1.2) rectangle (15.3, -4.8);
\node[lbl, anchor=north west] at (-0.2, 1.05) {Processor (chip)};

% --- CPU box ---
\draw[rounded corners=6pt, fill=red!5, thick]
  (0.1, -0.1) rectangle (4.3, -4.2);
\node[lbl, anchor=north west] at (0.3, -0.2) {CPU};

\node[comp, fill=red!12, minimum width=3.4cm] (gpr) at (2.2, -1.0)
  {GPR \\ \texttt{\$0}--\texttt{\$31}};
\node[comp, fill=red!12, minimum width=3.4cm] (alu) at (2.2, -2.3)
  {ALU \\ (integer arithmetic)};
\node[comp, fill=red!12, minimum width=1.4cm] (pc) at (1.3, -3.5)
  {PC};
\node[comp, fill=red!12, minimum width=1.4cm] (cu) at (3.1, -3.5)
  {Control};

% --- CP1 (FPU) box ---
\draw[rounded corners=6pt, fill=green!5, thick]
  (5.3, -0.1) rectangle (9.9, -4.2);
\node[lbl, anchor=north west] at (5.5, -0.2) {Coprocessor 1 (FPU)};

\node[comp, fill=green!12, minimum width=3.6cm] (fpr) at (7.6, -1.0)
  {FPR \\ \texttt{\$f0}--\texttt{\$f31}};
\node[comp, fill=green!12, minimum width=3.6cm] (fpalu) at (7.6, -2.5)
  {FP ALU \\ (FP arithmetic)};

% --- CP0 box ---
\draw[rounded corners=6pt, fill=orange!5, thick]
  (10.7, -0.1) rectangle (14.9, -4.2);
\node[lbl, anchor=north west] at (10.9, -0.2) {Coprocessor 0};
\node[font=\footnotesize, anchor=north west] at (10.9, -0.55) {(System Control)};

\node[comp, fill=orange!12, minimum width=3.2cm] (cp0) at (12.8, -1.7)
  {Status, Cause, EPC};

% --- mtc1/mfc1 arrows between GPR and FPR ---
\draw[arr, purple!70, <->] (gpr.east) -- (fpr.west)
  node[midway, above, font=\scriptsize] {\texttt{mtc1} / \texttt{mfc1}};

% ============ Memory (outside processor) ============
\draw[thick, rounded corners=6pt, fill=yellow!8]
  (0.5, -5.8) rectangle (10.3, -7.2);
\node[lbl, anchor=north west] at (0.7, -5.9) {Memory (RAM)};
\node[font=\small, align=center] at (5.4, -6.7)
  {Instructions + Data};

% --- Bus from CPU/CP1 through processor to memory ---
\draw[arr, thick, <->] (2.2, -4.2) -- (2.2, -5.8)
  node[midway, left, note] {\texttt{lw / sw}};
\draw[arr, thick, <->] (7.6, -4.2) -- (7.6, -5.8)
  node[midway, right, note] {\texttt{lwc1 / swc1}};

\end{tikzpicture}
\end{center}

\bigskip
\textbf{Which instruction talks to which component?}

\begin{center}
\begin{tabular}{lll}
\toprule
\textbf{Instruction} & \textbf{Path} & \textbf{Description} \\
\midrule
\texttt{add \$t0, \$t1, \$t2}
  & GPR $\to$ ALU $\to$ GPR
  & CPU internal \\
\texttt{add.s \$f0, \$f1, \$f2}
  & FPR $\to$ FP ALU $\to$ FPR
  & CP1 internal \\
\midrule
\texttt{lw \$t0, 0(\$t1)}
...
\texttt{sw \$t0, 0(\$t1)}
  ...
  & Store integer \\
\texttt{lwc1 \$f0, 0(\$t0)}
  & Memory $\to$ FPR (addr from GPR)
  & Load float \\
\texttt{swc1 \$f0, 0(\$t0)}
  & FPR $\to$ Memory (addr from GPR)
  & Store float \\
\midrule
\texttt{mtc1 \$t0, \$f0}
  & GPR $\to$ FPR
  & Transfer within processor \\
\texttt{mfc1 \$t0, \$f0}
  & FPR $\to$ GPR
  & Transfer within processor \\
\bottomrule
...
\begin{remark}
\texttt{lwc1} and \texttt{swc1} are \emph{cooperative} operations:
the CPU computes the memory address (using GPR + ALU), while the data
travels between Memory and the FP register file in CP1.
\end{remark}
\end{sub}
...
\begin{enumerate}
  ...
  \item Given the stored exponent byte $1001\;0110_2$, compute the true
        ...
        \hfill\hyperref[sol:6]{$\to$ Solution}
  ...
  \item Write a MIPS code fragment that loads two single-precision floats from
        ...
        \hfill\hyperref[sol:7]{$\to$ Solution}
\end{enumerate}
\end{sub}
...
\begin{enumerate}\setcounter{enumi}{7}
  ...
  \item Consider a 4-bit fraction (instead of 23).  What is the smallest
        ...
        exponent (bias $= 127$)?
        \hfill\hyperref[sol:11]{$\to$ Solution}

  \item True or false: in IEEE~754, $\text{NaN} = \text{NaN}$.
        Explain your answer.
        \hfill\hyperref[sol:12]{$\to$ Solution}
...
\subsection{Problem 1}\label{sol:1}
\begin{sub}
$12.375_{10}$:
\begin{enumerate}
...
\subsection{Problem 2}\label{sol:2}
\begin{sub}
...
\subsection{Problem 3}\label{sol:3}
\begin{sub}
\begin{enumerate}
...
\subsection{Problem 4}\label{sol:4}
\begin{sub}
$1.0_2 \times 2^{3} + 1.01_2 \times 2^{1}$:
\begin{enumerate}
  ...
  \item Result: $1010.1_2 = 10.5_{10}$.
\end{enumerate}
\end{sub}

\subsection{Problem 5}\label{sol:5}
\begin{sub}
$(-1.01_2 \times 2^{2}) \times (1.1_2 \times 2^{3})$:
\begin{enumerate}
  \item Sign: $1 \oplus 0 = 1$ (negative).
  ...
  \item Result: $-1.111_2 \times 2^{5} = -111100.0_2 = -60.0_{10}$.
\end{enumerate}
\end{sub}

\subsection{Problem 6}\label{sol:6}
\begin{sub}
...
\subsection{Problem 7}\label{sol:7}
\begin{sub}
\begin{verbatim}
...
\medskip
\textbf{Data flow overview:}

\begin{center}
\begin{tikzpicture}[
  box/.style={draw, minimum height=0.7cm, minimum width=2.2cm,
...
% --- Column labels ---
\node[lbl] at (0, 3.2)   {GPR};
\node[lbl] at (3.8, 3.2) {Memory};
\node[lbl] at (8, 3.2)   {FP Registers};

% --- GPR column ---
\node[box, fill=black!8] (t0) at (0, 2.4) {\$t0 = 100};
\node[box, fill=black!8] (t1) at (0, 1.5) {\$t1 = 200};
\node[box, fill=black!8] (t2) at (0, 0.6) {\$t2 = 300};

% --- Memory column ---
\node[box, fill=yellow!15] (m0) at (3.8, 2.4) {[100] \; 3.14};
\node[box, fill=yellow!15] (m1) at (3.8, 1.5) {[200] \; 2.50};
\node[box, fill=yellow!15] (m2) at (3.8, 0.6) {[300] \; 5.64};

% --- FP register column ---
\node[box, fill=green!15] (f0) at (8, 2.4) {\$f0 = 3.14};
\node[box, fill=green!15] (f1) at (8, 1.5) {\$f1 = 2.50};
\node[box, fill=blue!12]  (f2) at (8, 0.6) {\$f2 = 5.64};

% --- Arrows: GPR -> Memory (address lookup) ---
\draw[arr, dashed, black!50] (t0.east) -- (m0.west);
\draw[arr, dashed, black!50] (t1.east) -- (m1.west);
\draw[arr, dashed, black!50] (t2.east) -- (m2.west);

% --- Arrows: Memory <-> FP registers ---
\draw[arr, red!70!black] (m0.east) -- (f0.west)
  node[midway, above, note] {lwc1};
\draw[arr, red!70!black] (m1.east) -- (f1.west)
  node[midway, above, note] {lwc1};
\draw[arr, blue!70!black] (f2.west) -- (m2.east)
  node[midway, above, note] {swc1};

% --- add.s annotation ---
\draw[arr, black!70] (f0.south east) -- (f2.north east)
  node[midway, right, note, xshift=2pt] {add.s};
\draw[arr, black!70] (f1.south east) -- (f2.east)
  ;

% --- Legend ---
\node[anchor=north west, font=\scriptsize, text=black!70,
      align=left] at (0, -0.2) {%
  \tikz\draw[dashed, black!50, thick, -{Stealth[length=4pt]}] (0,0)--(0.6,0);
  \; address reference \qquad
  \tikz\draw[red!70!black, thick, -{Stealth[length=4pt]}] (0,0)--(0.6,0);
  \; load (Memory $\to$ FP reg) \qquad
  \tikz\draw[blue!70!black, thick, -{Stealth[length=4pt]}] (0,0)--(0.6,0);
  \; store (FP reg $\to$ Memory)
};

\end{tikzpicture}
\end{center}

\textbf{Operand ordering principle.}\quad
In MIPS, the \emph{register} operand always comes first:

\begin{center}
\begin{tabular}{lll}
\toprule
\textbf{Type} & \textbf{Pattern} & \textbf{Data flow} \\
\midrule
Arithmetic & \texttt{op \; dest, src1, src2} & $\texttt{src1} \circ \texttt{src2} \to \texttt{dest}$ \\
Load       & \texttt{op \; reg, \; offset(base)} & Memory $\to$ reg \\
Store      & \texttt{op \; reg, \; offset(base)} & reg $\to$ Memory \\
\bottomrule
\end{tabular}
\end{center}

Load and store have the \emph{same} operand order---register first, memory
address second.  The instruction name tells you the direction:
\textbf{l}oad reads \emph{into} the register; \textbf{s}tore writes
\emph{from} the register.

\medskip
\textbf{Addressing: \texttt{offset(\$base)}}\quad
means memory address $= \texttt{\$base} + \texttt{offset}$.
Writing \texttt{0(\$t0)} simply accesses the address stored in \texttt{\$t0}
(offset $= 0$, i.e.\ no shift).
\end{sub}

\subsection{Problem 8}\label{sol:8}
\begin{sub}
...
\subsection{Problem 9}\label{sol:9}
\begin{sub}
Use the repeated multiplication by 2 method (same as Section~5.1, Step~3):
...
\begin{array}{rcll}
  ...
  0.8 \times 2 &=& 1.6 & \to 1 \\
  0.6 \times 2 &=& 1.2 & \to 1 \\
  0.2 \times 2 &=& 0.4 & \to 0 \quad \leftarrow \text{back to } 0.2\text{; loop detected}
\end{array}
\]

Reading the integer parts top-to-bottom gives
...
\subsection{Problem 10}\label{sol:10}
\begin{sub}
...
\subsection{Problem 11}\label{sol:11}
\begin{sub}
...
\subsection{Problem 12}\label{sol:12}
\begin{sub}
...
\subsection{Problem 13}\label{sol:13}
\begin{sub}
...
\subsection{Problem 14}\label{sol:14}
\begin{sub}
...
%=============================================================================
% SECTION 13: Practice Worksheet (one problem per page with space for work)
%=============================================================================
\newpage
\section{Practice Worksheet}

\begin{secbody}
Each problem is printed on its own page with space for your handwritten solution.
\end{secbody}

% --- Problem 1 ---
\newpage
\subsection*{Problem 1 \hfill\normalsize\hyperref[sol:1]{$\to$ Solution}}
\begin{sub}
Convert $12.375_{10}$ to IEEE~754 single precision.  Give the final
32-bit representation in binary and hexadecimal.
\end{sub}
\vfill
...
% --- Problem 2 ---
\newpage
\subsection*{Problem 2 \hfill\normalsize\hyperref[sol:2]{$\to$ Solution}}
\begin{sub}
Convert the IEEE~754 single-precision value
\texttt{0xC1480000} to decimal.
\end{sub}
\vfill
...
% --- Problem 3 ---
\newpage
\subsection*{Problem 3 \hfill\normalsize\hyperref[sol:3]{$\to$ Solution}}
\begin{sub}
What decimal value does the following 32-bit pattern represent?
\[
  0\;\;10000010\;\;0110\;0000\;0000\;0000\;0000\;000
\]
\end{sub}
...
% --- Problem 4 ---
\newpage
\subsection*{Problem 4 \hfill\normalsize\hyperref[sol:4]{$\to$ Solution}}
\begin{sub}
Perform the following floating-point addition.  Show all steps
(align exponents, add significands, normalize, round).
\[
  1.0_2 \times 2^{3} \;+\; 1.01_2 \times 2^{1}
\]
...
% --- Problem 5 ---
\newpage
\subsection*{Problem 5 \hfill\normalsize\hyperref[sol:5]{$\to$ Solution}}
\begin{sub}
Perform the following floating-point multiplication.
\[
  -1.01_2 \times 2^{2} \;\times\; 1.1_2 \times 2^{3}
\]
\end{sub}
...
% --- Problem 6 ---
\newpage
\subsection*{Problem 6 \hfill\normalsize\hyperref[sol:6]{$\to$ Solution}}
\begin{sub}
Given the stored exponent byte $1001\;0110_2$, compute the true
exponent by adding $-127$ in binary.  Show the vertical addition.
\end{sub}
\vfill
...
% --- Problem 7 ---
\newpage
\subsection*{Problem 7 \hfill\normalsize\hyperref[sol:7]{$\to$ Solution}}

\textbf{Warm-up questions:}
\begin{sub}
\begin{enumerate}
  \item[(a)] What unit in MIPS handles floating-point operations?
        What are its registers called?

  \vspace{1.5cm}

  \item[(b)] What instruction loads a single-precision float from memory
        into an FP register? (i.e., the FP equivalent of \texttt{lw})

  \vspace{1.5cm}

  \item[(c)] What does \texttt{4(\$t0)} mean?
        If \texttt{\$t0} $= 200$, what memory address is accessed?

  \vspace{1.5cm}

  \item[(d)] What is the single-precision addition instruction?
        What instruction stores an FP register back to memory?
...
\textbf{Main problem:}
\begin{sub}
Write a MIPS code fragment that loads two single-precision floats from
memory addresses stored in \texttt{\$t0} and \texttt{\$t1}, adds them,
and stores the result to the address in \texttt{\$t2}.
\end{sub}
\vfill
...
% --- Problem 8 ---
\newpage
\subsection*{Problem 8 \hfill\normalsize\hyperref[sol:8]{$\to$ Solution}}
\begin{sub}
What is the IEEE~754 single-precision representation of $-0.0$?
How does it differ from $+0.0$?
\end{sub}
\vfill
...
% --- Problem 9 ---
\newpage
\subsection*{Problem 9 \hfill\normalsize\hyperref[sol:9]{$\to$ Solution}}
\begin{sub}
Convert $0.1_{10}$ to IEEE~754 single precision.  Why can this value
\emph{not} be represented exactly?
\end{sub}
\vfill
...
% --- Problem 10 ---
\newpage
\subsection*{Problem 10 \hfill\normalsize\hyperref[sol:10]{$\to$ Solution}}
\begin{sub}
What is the result of the following in IEEE~754?
\[
  (+\infty) + (-\infty) = \;?
\]
\end{sub}
...
% --- Problem 11 ---
\newpage
\subsection*{Problem 11 \hfill\normalsize\hyperref[sol:11]{$\to$ Solution}}
\begin{sub}
Consider a 4-bit fraction (instead of 23).  What is the smallest
positive \emph{denormalized} number representable with an 8-bit
exponent (bias $= 127$)?
\end{sub}
\vfill
...
% --- Problem 12 ---
\newpage
\subsection*{Problem 12 \hfill\normalsize\hyperref[sol:12]{$\to$ Solution}}
\begin{sub}
True or false: in IEEE~754, $\text{NaN} = \text{NaN}$.
Explain your answer.
\end{sub}
\vfill
...
% --- Problem 13 ---
\newpage
\subsection*{Problem 13 \hfill\normalsize\hyperref[sol:13]{$\to$ Solution}}
\begin{sub}
Show that floating-point addition is not associative by computing
$(a + b) + c$ and $a + (b + c)$ with
$a = 2^{24}$, $b = -2^{24}$, $c = 1.0$ in single precision.
Why do the results differ?
\end{sub}
...
% --- Problem 14 ---
\newpage
\subsection*{Problem 14 \hfill\normalsize\hyperref[sol:14]{$\to$ Solution}}
\begin{sub}
A student writes the MIPS instruction
\texttt{add.d~\$f1,~\$f2,~\$f4}.
What is wrong with this instruction?
\end{sub}
\vfill
