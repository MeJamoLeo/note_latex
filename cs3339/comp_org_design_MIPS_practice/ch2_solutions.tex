...
\texttt{B[8] = A[i-j];} with 4-byte (word) elements.
\begin{verbatim}
    sub  $t0, $s3, $s4     # $t0 = i - j
...
\texttt{\$t0} is a temporary register used for intermediate values.
\texttt{\$s0}--\texttt{\$s4} already hold the program variables
\texttt{f}--\texttt{j}, so overwriting any of them would destroy data
the program may still need.}

\medskip
{\small\textbf{Trace} (e.g.\ $i{=}5,\; j{=}2,\;
\texttt{\$s6}{=}\texttt{0x1000},\; \texttt{\$s7}{=}\texttt{0x2000},\;
A[3]{=}99$):}
...
\begin{tabular}{clll}
\toprule
\textbf{Step} & \textbf{Instruction} & \texttt{\$t0} & \textbf{Data flow} \\
\midrule
1 & \texttt{sub \$t0, \$s3, \$s4} & $5-2=3$
  & \texttt{\$s3}, \texttt{\$s4} $\to$ \texttt{\$t0} \\
2 & \texttt{sll \$t0, \$t0, 2}    & $3\times4=12$
  & \texttt{\$t0} $\to$ \texttt{\$t0} \\
3 & \texttt{add \$t0, \$s6, \$t0} & \texttt{0x100C}
  & \texttt{\$s6}$+$\texttt{\$t0} $\to$ \texttt{\$t0} \\
4 & \texttt{lw \$t0, 0(\$t0)}     & $99$
  & Mem[\texttt{0x100C}] $\to$ \texttt{\$t0} \\
5 & \texttt{sw \$t0, 32(\$s7)}    & $99$
  & \texttt{\$t0} $\to$ Mem[\texttt{0x2020}] \\
\bottomrule
\end{tabular}
\end{center}
...
\begin{enumerate}
  ...
  \item \texttt{lw \$s0, 0(\$t0)} $\rightarrow$ \texttt{\$s0 = A[f]}
        \quad$\Leftarrow$ \textbf{\texttt{\$s0} is overwritten:
        the original value of \texttt{f} is lost from this point on.}
  \item \texttt{addi \$t2, \$t0, 4} $\rightarrow$ \texttt{\$t2 = \&A[f] + 4
        = \&A[f+1]}
  \item \texttt{lw \$t0, 0(\$t2)} $\rightarrow$ \texttt{\$t0 = A[f+1]}
  \item \texttt{add \$t0, \$t0, \$s0} $\rightarrow$ \texttt{\$t0 = A[f+1]
        + A[f]} \quad (note: \texttt{\$s0} is now A[f], not f)
  \item \texttt{sw \$t0, 0(\$t1)} $\rightarrow$ \texttt{B[g] = A[f+1] + A[f]}
\end{enumerate}

C code (using the original value of f before it is destroyed):
\begin{verbatim}
    B[g] = A[f] + A[f + 1];
\end{verbatim}

{\scriptsize\textit{Caveat:}
Unlike the C statement above, the assembly has a side effect:
\texttt{\$s0}~(\texttt{f}) is overwritten with \texttt{A[f]} at step~5.
In C this would be equivalent to:
\texttt{f = A[f]; B[g] = f + A[f\_orig + 1];}
where \texttt{f\_orig} is the original value of \texttt{f} (already used
to compute the address before step~5).}
...
\scriptsize
\textbf{Background --- Endianness:}\quad
Endianness refers to the ordering of bytes within a word in memory.
\textbf{Little-endian} (Intel/DEC): most significant byte stored at the
highest address.
\textbf{Big-endian} (IBM/Motorola): most significant byte stored at the
lowest address.
Example --- the value \texttt{0x00000003} at address \texttt{0x4000}:

\begin{center}
\begin{tabular}{lcccc}
\toprule
& \texttt{x4003} & \texttt{x4002} & \texttt{x4001} & \texttt{x4000} \\
\midrule
Little-endian & \texttt{00} & \texttt{00} & \texttt{00} & \texttt{03} \\
Big-endian    & \texttt{03} & \texttt{00} & \texttt{00} & \texttt{00} \\
\bottomrule
\end{tabular}
\end{center}
\end{minipage}
...
\textit{Alt (Horner's method):} Evaluate left-to-right by repeated
$\times 16 +{}$next digit --- no large powers needed:

\begin{center}
\small
\begin{tabular}{rl}
\toprule
\textbf{Digit} & \textbf{Accumulator} \\
\midrule
A $(10)$ & $10$ \\
B $(11)$ & $10 \times 16 + 11 = 171$ \\
C $(12)$ & $171 \times 16 + 12 = 2{,}748$ \\
D $(13)$ & $2{,}748 \times 16 + 13 = 43{,}981$ \\
E $(14)$ & $43{,}981 \times 16 + 14 = 703{,}710$ \\
F $(15)$ & $703{,}710 \times 16 + 15 = 11{,}259{,}375$ \\
1 $(1)$  & $11{,}259{,}375 \times 16 + 1 = 180{,}150{,}001$ \\
2 $(2)$  & $180{,}150{,}001 \times 16 + 2 = \mathbf{2{,}882{,}400{,}018}$ \\
\bottomrule
\end{tabular}
\end{center}
...
Elements are 8 bytes but MIPS registers are 32 bits (4 bytes), so each
element requires two \texttt{lw}/\texttt{sw} operations (upper and lower
word). The index is multiplied by 8 (shift left by 3). Assuming big-endian
(upper word at offset 0, lower word at offset 4):
\begin{verbatim}
    # Load A[i] (64-bit)
    sll  $t0, $s3, 3       # $t0 = i * 8
    add  $t0, $s6, $t0     # $t0 = &A[i]
    lw   $t1, 0($t0)       # $t1 = A[i] upper 32 bits
    lw   $t2, 4($t0)       # $t2 = A[i] lower 32 bits

    # Load A[j] (64-bit)
    sll  $t3, $s4, 3       # $t3 = j * 8
    add  $t3, $s6, $t3     # $t3 = &A[j]
    lw   $t4, 0($t3)       # $t4 = A[j] upper 32 bits
    lw   $t5, 4($t3)       # $t5 = A[j] lower 32 bits

    # 64-bit addition: lower + carry into upper
    addu $t6, $t2, $t5     # $t6 = lower sum
    sltu $t7, $t6, $t2     # $t7 = carry (1 if overflow)
    addu $t1, $t1, $t4     # $t1 = upper sum (no carry yet)
    addu $t1, $t1, $t7     # $t1 = upper sum + carry

    # Store to B[8] (offset = 8*8 = 64)
    sw   $t1, 64($s7)      # B[8] upper 32 bits
    sw   $t6, 68($s7)      # B[8] lower 32 bits
\end{verbatim}
\textit{Why \texttt{addu} instead of \texttt{add}?}\enspace
\texttt{add} raises an overflow exception, while \texttt{addu} does not.
Here the lower-word addition is expected to overflow (the carry is
propagated to the upper word via \texttt{sltu}), so \texttt{addu} is used
to prevent an unwanted exception.

\medskip\noindent\textit{Instruction templates:}
\begin{itemize}
  \item \texttt{addu} (Add Unsigned) --- \texttt{addu \$rd, \$rs, \$rt}:
        \texttt{\$rd = \$rs + \$rt} (no overflow exception)
  \item \texttt{sltu} (Set Less Than Unsigned) --- \texttt{sltu \$rd, \$rs, \$rt}:
        \texttt{\$rd = (\$rs < \$rt) ? 1 : 0} (used here to detect carry)
\end{itemize}
...
\midrule
Dec & 0 & 16 (\$s0) & 17 (\$s1) & 16 (\$s0) & 0 & 32 (add) \\
Bin & \texttt{000000} & \texttt{10000} & \texttt{10001}
    & \texttt{10000} & \texttt{00000} & \texttt{100000} \\
Hex & \texttt{0x00} & \texttt{0x10} & \texttt{0x11}
    & \texttt{0x10} & \texttt{0x00} & \texttt{0x20} \\
\bottomrule
\end{tabular}
\end{center}
Full word: \texttt{0000\,0010\,0001\,0001\,1000\,0000\,0010\,0000}
$=$ \texttt{0x02118020}
...
\midrule
Dec & 43 (sw) & 16 (\$s0) & 9 (\$t1) & 0 \\
Bin & \texttt{101011} & \texttt{10000} & \texttt{01001}
    & \texttt{0000\,0000\,0000\,0000} \\
Hex & \texttt{0x2B} & \texttt{0x10} & \texttt{0x09}
    & \texttt{0x0000} \\
\bottomrule
\end{tabular}
\end{center}
Full word: \texttt{1010\,1110\,0000\,1001\,0000\,0000\,0000\,0000}
$=$ \texttt{0xAE090000}
...
the lower 32~bits are \texttt{0x50000000}.
However, \texttt{add} is a \emph{signed} operation that \textbf{traps on
overflow}.  Both operands are negative yet the 32-bit result is positive,
so overflow is detected: the CPU raises an exception and
\textbf{\texttt{\$t0} is not written} (the ALU result is discarded).

\noindent\textbf{2.10.2}
True sum: $-2{,}147{,}483{,}648 + (-805{,}306{,}368) = -2{,}952{,}789{,}016$,
which is below $-2^{31} = -2{,}147{,}483{,}648$.
\textbf{Overflow has occurred.}
Because \texttt{add} (not \texttt{addu}) was used, the processor traps
and \texttt{\$t0} retains its prior value.
...
= -1{,}342{,}177{,}280$, which fits in $[-2^{31},\;2^{31}-1]$.
No overflow $\Rightarrow$ \texttt{\$t0 = 0xB0000000} is written normally.

\noindent\textbf{2.10.4}
\textbf{No overflow.}
The result $-1{,}342{,}177{,}280$ is representable in 32-bit signed,
so \texttt{sub} completes without exception.

\phantomsection\addcontentsline{toc}{subsubsection}{2.10.5}
\medskip\noindent\textbf{2.10.5}
First instruction: \texttt{add \$t0, \$s0, \$s1} --- as shown in 2.10.1,
this overflows.  \texttt{add} traps on overflow, so:
\begin{itemize}
  \item The CPU raises an exception and jumps to the exception handler.
  \item \texttt{\$t0} is \textbf{not updated}.
  \item The second instruction \texttt{add \$t0, \$t0, \$s0} is
        \textbf{never executed}.
\end{itemize}
Therefore \texttt{\$t0} remains at its prior value (unspecified here).

\begin{flushright}
\begin{minipage}{0.92\linewidth}
\scriptsize
\textbf{Note:}\quad
If \texttt{addu} were used instead of \texttt{add}, no exception would
occur and both instructions would execute:
\texttt{0x80000000}~$+$~\texttt{0xD0000000}~$=$~\texttt{0x50000000},
then \texttt{0x50000000}~$+$~\texttt{0x80000000}~$=$~\texttt{0xD0000000}.
The final value \texttt{0xD0000000} would happen to equal \texttt{\$s1},
but the computation would still be arithmetically incorrect.
\end{minipage}
\end{flushright}

\noindent\textbf{2.10.6}
\textbf{Overflow occurred} in the first \texttt{add}.
The exception prevents the second instruction from running,
so no ``desired result'' is produced at all.
...
\texttt{\$s1} = 128 = \texttt{0x80}.\quad
Signed overflow occurs when the result falls outside
$[-2^{31},\; 2^{31}-1]$.

%--- 2.11.1 ---
\medskip\noindent\textbf{2.11.1} \texttt{add \$s0, \$s0, \$s1}

\noindent\textit{Positive overflow}
($\texttt{\$s0} + 2^{7} > 2^{31}-1$):
\[
  \texttt{\$s0} > 2^{31}-1-2^{7}
  \;\Longrightarrow\;
  \texttt{\$s0} \ge 2^{31}-2^{7}
\]
$\texttt{\$s0} \in [2^{31}-2^{7},\; 2^{31}-1]
= [\texttt{0x7FFFFF80},\; \texttt{0x7FFFFFFF}]$.

\smallskip
\noindent\textit{Negative overflow}
($\texttt{\$s0} + 2^{7} < -2^{31}$):
$\texttt{\$s0} < -2^{31}-2^{7}$,
but $\texttt{\$s0} \ge -2^{31}$ always, so \textbf{impossible}.

%--- 2.11.2 ---
\medskip\noindent\textbf{2.11.2} \texttt{sub \$s0, \$s0, \$s1}
...
\[
  \texttt{\$s0} < -2^{31}+2^{7}
  \;\Longrightarrow\;
  \texttt{\$s0} \le -2^{31}+2^{7}-1
\]
$\texttt{\$s0} \in [-2^{31},\; -2^{31}+2^{7}-1]
= [\texttt{0x80000000},\; \texttt{0x8000007F}]$.

\smallskip
\noindent\textit{Positive overflow}
($\texttt{\$s0} - 2^{7} > 2^{31}-1$):
$\texttt{\$s0} > 2^{31}-1+2^{7}$,
but $\texttt{\$s0} \le 2^{31}-1$ always, so \textbf{impossible}.

%--- 2.11.3 ---
\medskip\noindent\textbf{2.11.3} \texttt{sub \$s0, \$s1, \$s0}

Result is $2^{7} - \texttt{\$s0}$.

\smallskip
\noindent\textit{Positive overflow}
($2^{7} - \texttt{\$s0} > 2^{31}-1$):
\[
  \texttt{\$s0} < 2^{7}-(2^{31}-1) = -2^{31}+2^{7}+1
  \;\Longrightarrow\;
  \texttt{\$s0} \le -2^{31}+2^{7}
\]
$\texttt{\$s0} \in [-2^{31},\; -2^{31}+2^{7}]
= [\texttt{0x80000000},\; \texttt{0x80000080}]$.
...
\begin{verbatim}
  000000 10000 10100 10000 00000 100000
...
\scriptsize
\textbf{Note:}\quad
The \texttt{op} field is 6~bits (bits [31:26]), so valid opcodes
are \texttt{0x00}--\texttt{0x3F} (0--63).
\texttt{op=0x23} = \texttt{100011}$_{2}$ (6~bits; the 8-bit form
\texttt{00100011} has two leading zero-padding bits).
A value such as \texttt{op=0x43} (=67, requires 7~bits) is
\textbf{impossible} since it exceeds the 6-bit field.
\end{minipage}
\end{flushright}
...
\begin{align*}
  \texttt{srl \$t2, \$t0, 4}: &\quad \texttt{0xAAAAAAAA} \gg 4
    = \texttt{0x0AAAAAAA} \\
  \texttt{andi \$t2, \$t2, -1}: &\quad \text{andi \textbf{zero}-extends the
    16-bit immediate } \texttt{-1} = \texttt{0xFFFF} \\
...
\begin{minipage}{0.92\linewidth}
\scriptsize
\textbf{Note 1 --- Why $-1$ = \texttt{0xFFFF}, not \texttt{1001}}\par\smallskip
MIPS integers use \textbf{2's complement}, not sign-magnitude.
In sign-magnitude, $-1$ would be a sign bit followed by the value~1
(e.g.\ \texttt{1\,001}).
In 2's complement:
\begin{verbatim}
   +1 = 0000 0000 0000 0001
  ~+1 = 1111 1111 1111 1110   (bit-flip)
   +1 = 1111 1111 1111 1111   (add 1)  = 0xFFFF
\end{verbatim}
\vspace{-6pt}
(Intuition: \texttt{0x0000 $-$ 0x0001} wraps to \texttt{0xFFFF}.)

\smallskip
Note: IEEE~754 floating point \emph{does} use sign-magnitude
(bit~31 = sign, remaining bits = exponent + mantissa).
Only integer arithmetic in MIPS uses 2's complement.

\medskip
\textbf{Note 2 --- zero-extend vs.\ sign-extend}\par\smallskip
The 16-bit immediate \texttt{0xFFFF} has bit~15~=~1.
If \emph{sign}-extended, bits [31:16] would all be~1
$\Rightarrow$ \texttt{0xFFFFFFFF}.
But \texttt{andi} \textbf{zero}-extends, so bits [31:16] are all~0
$\Rightarrow$ \texttt{0x0000FFFF}.

\smallskip
\begin{tabular}{@{}l l@{}}
\toprule
Sign-extend (\texttt{addi}, \texttt{lw}, \texttt{sw}, \texttt{beq}, \ldots)
  & bit~15 copied to bits [31:16] \\
Zero-extend (\texttt{andi}, \texttt{ori}, \texttt{xori})
  & bits [31:16] $= 0$ \\
\bottomrule
\end{tabular}
\end{minipage}
...
\noindent\textbf{2.22.1} \texttt{jal} performs two operations
(Verilog: \texttt{R[31]=PC+8; PC=JumpAddr}):

\begin{enumerate}
\item \textbf{Jump} --- target address (32~bits) is formed by
  concatenating three parts:
\begin{verbatim}
  [31:28]  [27:2]          [1:0]
  +------+------------------+----+
  | PC+4 |     imm26        | 00 |   = JumpAddr
  | 4bit |     26bit        | 2bit|
  +------+------------------+----+
\end{verbatim}
  \vspace{-6pt}
  The upper 4~bits come from PC+4 (the current 256\,MB region).
  The lower 2~bits are always \texttt{00} (word-aligned).
  Range: any word-aligned address in the same 256\,MB region as PC+4:
  \[
    [\texttt{0x00000000},\; \texttt{0x0FFFFFFC}]
  \]
\item \textbf{Link} (\textbf{not relevant to this problem} --- included
  as background) --- saves the return address in
  \texttt{\$ra}\,(\texttt{\$31}).
  The return address is \textbf{PC+8} (not PC+4), because of the
  \emph{branch delay slot}: the instruction at PC+4 always executes
  before the jump takes effect, so the caller resumes at PC+8.
  This does not affect the \emph{range} of addresses \texttt{jal}
  can jump to.
\end{enumerate}
\begin{verbatim}
  PC    :  jal target      <- this instruction
  PC+4  :  add ...         <- delay slot (always executes)
  PC+8  :  ...             <- return here ($ra = PC+8)
\end{verbatim}

\noindent\textbf{2.22.2} \texttt{beq} uses a 16-bit \textbf{signed}
offset (I-type immediate).  The offset is in \emph{words} (shifted
left by~2), and relative to PC+4:
\[
  \text{BranchAddr} = \text{PC}+4 + \text{sign\_ext}(\textit{imm16}) \times 4
\]

\begin{verbatim}
  [31:26] [25:21] [20:16] [15:0]
  +------+-------+-------+------------------+
  | beq  |  rs   |  rt   | imm16 (signed)   |
  | 6bit |  5bit |  5bit | 16bit            |
  +------+-------+-------+------------------+
\end{verbatim}
\vspace{-6pt}

The 16-bit signed immediate ranges from $-2^{15}$ to $+2^{15}-1$
($-32{,}768$ to $+32{,}767$ words).
Multiplied by~4 (word $\to$ byte):
$-2^{17}$ to $+2^{17}-4$ bytes ($\pm 128$\,KB).

\begin{align}
  \text{Lower} &= \text{PC}+4 + (-2^{17})
    = \texttt{0x00400004} - \texttt{0x20000}
    = \texttt{0x003E0004} \notag\\
  \text{Upper} &= \text{PC}+4 + (2^{17}-4)
    = \texttt{0x00400004} + \texttt{0x1FFFC}
    = \texttt{0x00420000} \notag
\end{align}
\[
  \text{Range} = [\texttt{0x003E0004},\; \texttt{0x00420000}]
\]
(\texttt{beq} also has a branch delay slot.)

\begin{flushright}
\begin{minipage}{0.92\linewidth}
\scriptsize
\textbf{Note: \texttt{jal} vs.\ \texttt{beq} --- addressing modes}\par\smallskip
Both multiply the stored value by~4 ($\times 4$, i.e.\ append
\texttt{00}) because all MIPS instructions are word-aligned.
However, the address \emph{calculation} is fundamentally different:

\smallskip
\begin{tabular}{@{}l l l@{}}
\toprule
& \texttt{jal} (pseudo-direct) & \texttt{beq} (PC-relative) \\
\midrule
Operation & bit concatenation & addition \\
Formula &
  \texttt{\{PC+4[31:28], imm26, 00\}} &
  $\text{PC+4} + \text{sign\_ext}(\textit{imm16}) \times 4$ \\
Backward jump? & No (no sign concept) & Yes (signed offset) \\
\bottomrule
\end{tabular}

\smallskip
\texttt{beq} uses addition with a signed offset because loops need
to branch \emph{backward}. Concatenation has no sign concept and
cannot express a negative direction.

\smallskip
Example --- \texttt{beq} with \texttt{imm16 = 0xFFFF} ($-1$):
$\text{sign\_ext}(\texttt{0xFFFF}) \times 4 = -4$,
so $\text{BranchAddr} = \text{PC}+4-4 = \text{PC}$
(jumps back to itself --- infinite loop).
\end{minipage}
\end{flushright}
