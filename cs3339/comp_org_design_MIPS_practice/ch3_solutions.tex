...
Check range: 12-bit sign-magnitude range is $[-(2^{11}-1),\; +(2^{11}-1)] =
...
Since $63 \in [0, 255]$ and $185 \ge 122$ (no borrow out), there is
...
Since $|-179| = 179 > 127$, this \textbf{overflows} (negative overflow, i.e.,
the result is more negative than the format can represent).
...
Since $65 \in [-127, +127]$, there is \textbf{no overflow}.
...
In signed 8-bit two's complement, the bit pattern for a decimal value $d$
(where $0 \le d \le 255$) is interpreted as:
\begin{itemize}
  \item $151_{10} = 10010111_2$. Since MSB $= 1$: value $= 151 - 256 = -105$.
  \item $214_{10} = 11010110_2$. Since MSB $= 1$: value $= 214 - 256 = -42$.
\end{itemize}

True sum:
\[ (-105) + (-42) = -147 \]

Signed 8-bit two's complement range: $[-128, +127]$.  Since $-147 < -128$,
overflow occurs (negative overflow).

With \textbf{saturating arithmetic}, the result clamps to the most negative
representable value: $\mathbf{-128}$.
...
Same interpretations: $151 \to -105$, $214 \to -42$.

\[ 151 - 214 \;\text{ means }\; (-105) - (-42) = -105 + 42 = -63 \]

Since $-63 \in [-128, +127]$, no overflow occurs. Saturating arithmetic has no
effect.
